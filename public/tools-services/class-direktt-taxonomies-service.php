...
class Direktt_Taxonomies_Service {
	...
	public function direktt_taxonomies_service_add_shortcode() {
		add_shortcode( 'direktt_edit_taxonomies_service', array( $this, 'direktt_taxonomies_service_shortcode' ) );
	}
	...
	public function direktt_taxonomies_service_shortcode() {
		...
		if ( $subpage ) {
			...
			if ( isset( $_POST['save_user_categories'] ) ) {
				if (
					...
					wp_send_json( array( 'status' => 'nonce_failed' ) );
				}
				...
				if ( ! $id_to_add ) {
					wp_send_json( array( 'status' => 'no_user' ) );
				} else {
				...
				<div class="direktt-edit-taxonomies-service-wrapper direktt-edit-taxonomies-service-editor" id="direktt-profile">
					...
					if ( ( 'edit-category' === $subpage || 'edit-tag' === $subpage ) && isset( $_GET['tax_name'] ) ) {
						...
						<?php if ( ! empty( $status_message ) ) : ?>
							<p class="direktt-edit-taxonomies-service-status notice"><?php echo esc_html( $status_message ); ?></p>
						<?php endif; ?>
						...
						<p><a href="<?php echo esc_url( $back_uri ); ?>" class="direktt-button button-invert button-dark-gray"><?php echo esc_html__( 'Show All Taxonomies', 'direktt' ); ?></a></p>
						<?php
		...
		<div id="direktt-profile-wrapper">
			...
			foreach ( $all_categories as $category ) {
				...
				?>
						<p><a href="<?php echo esc_url( $new_uri ); ?>" class="direktt-button"><?php echo esc_html( $category['name'] ); ?><?php echo ' <i>(' . esc_html( $count ) . ')</i>'; ?></a></p>
					<?php
			}
			?>
				</div>
				...
				<div class="direktt-edit-taxonomies-service-tags">
					...
					foreach ( $all_tags as $tag ) {
						...
						?>
						<p><a href="<?php echo esc_url( $new_uri ); ?>" class="direktt-button"><?php echo esc_html( $tag['name'] ); ?><?php echo ' <i>(' . esc_html( $count ) . ')</i>'; ?></a></p>
						<?php
					}
					?>
				</div>
